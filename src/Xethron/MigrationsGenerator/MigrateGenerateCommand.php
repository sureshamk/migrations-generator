...
class MigrateGenerateCommand extends GeneratorCommand
{
    ...
    public function fire()
    {
        ...
        if ($this->log) {
            ...
            $this->batch = $this->askNumeric('Next Batch Number is: ' . $batch . '. We recommend using Batch Number 0 so that it becomes the "first" migration',
                0);
        }
    ...
    protected function getFileGenerationPath()
    {
        ...
        $fileName = str_replace('prefix_', '', $fileName);
    ...
    {
        //$this->table = str_replace('mycloudbin_', '', $this->table);
        $this->migrationName = str_replace('prefix_', '', $this->migrationName);
        if ($this->method == 'create') {
            $up = (new AddToTable($this->file, $this->compiler))->run($this->fields, $this->table, $this->connection,
                'create');
